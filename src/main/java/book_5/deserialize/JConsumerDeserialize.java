...
public class JConsumerDeserialize extends Thread {
	...
	private Properties configure() {
		...
		props.put("bootstrap.servers", "kafka1:9092,kafka2:9092,kafka:9092");// 指定Kafka集群地址
		props.put("group.id", "test_deserializer");// 指定消费者组
		props.put("enable.auto.commit", "true");// 开启自动提交
		props.put("auto.commit.interval.ms", "1000");// 自动提交的时间间隔
		// 反序列化消息主键
		props.put("key.deserializer", "org.apache.kafka.common.serialization.StringDeserializer");
		// 反序列化消费记录
		props.put("value.deserializer", "book_5.deserialize.JSalaryDeserializer");
		return props;
	}
	...
	public void run() {
		...
		while (flag) {
			...
			for (ConsumerRecord<String, String> record : records)
				// 循环打印消息记录  offset = 5, key = key, value = JSalarySerial [id=2021, salary=100]
